...
tilesize parse_picture_name_to_coordinates(const string filepath) {
  // /autox/web/vwd/dataset/user/jsun@autox.ai/TopViewMaps/-134.941437_-182.526642_214.169428.png/annotation/index.json
  ...
  replace(x_y_scale.begin(), x_y_scale.end(), '_', ' '); // => "x y scale"

  // convert string to 3 double numbers
  string::size_type sz; // alias of size_t
  double x_base = stod(x_y_scale, &sz);
  double y_base = stod(x_y_scale.substr(sz));
  double scale = stod(x_y_scale.substr(sz));
...
int main() {
  ...
  string json_file_path("/autox/web/vwd/dataset/user/jsun@autox.ai/TopViewMaps/"
                        "-134.941437_-182.526642_214.169428.png/annotation/"
                        "index.json");
  ...
  // for (auto &p : polygons) {
  //   cout << "polygon: " << p.name << " points " << p.points.size() << '\n';
  // }
